...
export function changeItem(id) {
    ...
    TweenMax.to('.main-presentation-shadow', 1, { backgroundColor:'rgba(0, 0, 0, 0.5)' }).delay(1);
...
export function switchToMain() {
    ...
    TweenMax.to('.main-presentation-shadow', .5, { backgroundColor:'#000' }  ).delay(2);
    TweenMax.to('.the_content-presentation', .5, { display:'none', opacity:0 }  ).delay(2);
    TweenMax.to('.contact-phones', .5, { opacity: 0 }  ).delay(2);
    TweenMax.to('.main-presentation-shadow', .5, { backgroundColor:'rgba(0, 0, 0, 0.3)' }  ).delay(3);

    setTimeout( () => {
        TweenMax.to('#area-for-header-svg', .5, { opacity: 1, display:'block' })
        TweenMax.to('.the-content-description', .5, { opacity: 1 })
        TweenMax.to('.the-content-description', .5, { display: 'block' }  ).delay(0.5)
